...
// 配置固定值
const FIXED_VALUES = {
  PIXEL_STREAMING_IP: "192.168.110.108",
};

// 工具函数：替换字符串中的占位符
const replacePlaceholdersInString = (value, usedPorts, portMap) => {
  return value.replace(/\$\{([a-zA-Z0-9_]+)\}/g, (_, placeholder) => {
    if (FIXED_VALUES[placeholder]) {
      return FIXED_VALUES[placeholder];
    }
...
async function replacePlaceholders(config, usedPorts) {
  ...
  // 替换顶层配置中的占位符
  if (config.ip) {
    config.ip = replacePlaceholdersInString(config.ip, usedPorts, portMap);
  }

  if (config.port) {
    config.port = replacePlaceholdersInString(config.port, usedPorts, portMap);
  }
  ...
  for (let image of config.images) {
    // 替换 ports 中的占位符
    if (image.ports) {
      image.ports = replacePlaceholdersInString(image.ports, usedPorts, portMap);
    }

    // 替换 env 中的占位符和变量名中的前缀 "-"
    if (image.env) {
      image.env = image.env
        .split(' ')
        .map(item => {
          ...
          return `${name}=${value ? replacePlaceholdersInString(value, usedPorts, portMap) : ''}`;
        })
        .join(' ');
    }
  }
