...
def _ensure_client():
    ...
    """
    确保Langfuse客户端被正确初始化的函数
    如果初始化失败或未设置必要的环境变量，则返回None
    """
    global _client, _init_failed  # 声明全局变量，用于跟踪客户端状态和初始化状态
    if _init_failed:  # 如果初始化失败过，直接返回None
        return None
    if _client is None:  # 如果客户端尚未初始化
        try:
            from langfuse import Langfuse  # 导入Langfuse SDK
            # 从环境变量中获取公钥和私钥
            ...
            if not (pk and sk):
                ...
                logger.debug("langfuse disabled: LANGFUSE_PUBLIC_KEY / SECRET_KEY not set")  # 记录调试信息
                return None
