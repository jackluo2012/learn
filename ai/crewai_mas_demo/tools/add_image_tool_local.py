...
def _local_path_to_base64_data_and_compress_url(image_url: str) -> str | None:
    """
    将本地图片文件路径转换为 Base64 格式的 Data URL。

    该函数首先解析并验证输入的路径是否为存在的本地文件。如果是，则读取文件内容，
    根据文件后缀确定 MIME 类型，并将二进制数据编码为 Base64 字符串，最终组装成
    Data URL 格式返回。如果路径无效或文件不存在，则返回错误信息字符串。
    注意：当前的图片压缩逻辑已被注释禁用。

    参数:
        image_url (str): 本地图片文件的路径字符串，支持波浪号 (~) 展开用户主目录。

    返回:
        str | None: 若成功转换，返回格式为 "data:{mime_type};base64,{encoded_data}" 的字符串；
                    若文件不存在，返回格式为 "图片文件不存在：{image_url}" 的错误提示字符串。
    """
