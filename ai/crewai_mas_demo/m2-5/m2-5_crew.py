...
class ImageAnalysis(BaseModel):
    ...
    subject_description: str = Field(
        ..., description="【主体内容】客观描述画面中的核心物体、人物或场景。"
    )
    atmosphere_vibe: str = Field(
        ..., description="【风格氛围】用形容词描述画面的情绪价值。"
    )
    visual_details: List[str] = Field(
        ..., description="【细节点列表】列出画面中容易被忽略但具象的元素。"
    )
    image_quality_score: str = Field(
        ..., description="【质量评价】1-10分打分，基于构图、光线和清晰度。"
    )
    highlight_feature: str = Field(
        ..., description="【突出特点】这张图最抓人眼球的一个视觉锚点（Visual Hook）。"
    )
...
class VisualAnalysisReport(BaseModel):
    ...
    analyzed_images: List[ImageAnalysis] = Field(
        ..., description="包含所有输入图片的详细分析列表。"
    )
    overall_visual_summary: str = Field(
        ..., description="综合所有图片得出的整体视觉基调总结。"
    )
...
class ContentStrategyBrief(BaseModel):
    ...
    input_evaluation: str = Field(
        ...,
        description="【素材评估】基于用户诉求和图片质量的综合评价，指出优势和劣势，并给出修图建议。",
    )
    target_audience_persona: str = Field(
        ...,
        description="【目标受众画像】采用反漏斗模型，定义最核心的细分人群（年龄段、职业标签、生活状态、心理诉求）。",
    )
    core_pain_point: str = Field(
        ..., description="【核心痛点/诉求】受众最想解决的问题或最渴望的情绪价值。"
    )
    suggested_title: str = Field(
        ...,
        description="【建议标题】遵循公式：'痛点场景 + 情绪/利益钩子 + 核心人群标签'，包含标点符号和Emoji，20字以内。",
    )
    content_outline: List[str] = Field(
        ...,
        description="【笔记大纲】笔记正文的结构安排（场景引入、沉浸式体验、干货植入、结尾强引导）。",
    )
    engagement_strategy: str = Field(
        ..., description="【点赞评论诱饵】设计具体的策略来引发评论互动。"
    )
    retention_strategy: str = Field(
        ..., description="【收藏诱饵】提供具体的实用价值让用户点击收藏。"
    )
    seo_keywords: List[str] = Field(
        ...,
        description="【关键词布局】基于 KFS 策略，列出 3 个必须埋入文案的长尾关键词。",
    )
...
class CopywritingOutput(BaseModel):
    ...
    title: str = Field(
        ..., description="【笔记标题】完整的小红书笔记标题，包含Emoji和标点符号。"
    )
    content: str = Field(
        ...,
        description="【笔记正文】完整的小红书笔记正文内容，按照策略简报中的内容大纲撰写。",
    )
    picture_list: List[ImageAnalysis] = Field(
        ...,
        description="【图片列表】根据视觉元素描述筛选合适的图片并按照合理的顺序排序，特别重视第一张图片的选择。",
    )
...
class SEOOptimizedNoteReport(BaseModel):
    ...
    optimization_summary: str = Field(
        ...,
        description="【优化总结】说明本次SEO优化的重点和改进点，包括关键词密度、自然度、搜索占位策略等。",
    )
    optimized_title: str = Field(
        ...,
        description="【优化后的标题】在原始标题基础上进行SEO优化的结果，要保持原来的风格。",
    )
    optimized_content: str = Field(
        ...,
        description="【优化后的正文】在原始正文基础上进行SEO优化，自然融入长尾关键词，确保关键词密度合理，不影响阅读体验。",
    )
    optimized_picture_list: List[ImageAnalysis] = Field(
        ...,
        description="【优化后的图片列表】根据优化后的正文，筛选合适的图片并按照合理的顺序排序。",
    )
